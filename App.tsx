...
import { Item, JobInfo, WindowType } from './types';
  ...
  return (
    ...
    >
        ...
        <div className="p-4 md:p-6 pb-2 md:pb-6 overflow-y-auto no-scrollbar">
          <div className="flex justify-between items-center mb-6 hidden md:flex">
             <div className="flex items-center gap-3">
                <div className="relative w-10 h-10 rounded-full flex items-center justify-center border-2 border-white shadow-lg overflow-hidden shrink-0" style={{ backgroundColor: '#d6001c' }}>
                  <span className="text-white font-black text-xl leading-none select-none tracking-tighter" style={{ fontFamily: 'Inter, sans-serif' }}>84</span>
                </div>
          ...
          <div className="flex md:hidden justify-between items-center mt-3">
             ...
             <div className="flex gap-1 items-center">
               ...
               <button onClick={clearAll} className="p-2 text-zinc-600 hover:text-red-400"><Trash2 size={16} /></button>
               <button onClick={exportCSV} className="ml-1 flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] font-bold transition-all border" style={{ backgroundColor: 'color-mix(in srgb, var(--accent), transparent 90%)', borderColor: 'color-mix(in srgb, var(--accent), transparent 60%)', color: 'var(--accent)' }}>
      ...
      <main className="flex-1 flex flex-col bg-zinc-950 relative overflow-hidden">
        <div className="hidden md:flex items-center justify-between p-6 pb-2 shrink-0">
          <div className="flex items-center gap-4">
             <span className="text-sm font-bold text-zinc-400 uppercase tracking-widest">Master Inventory</span>
             <span className="bg-zinc-800 text-zinc-500 text-[10px] px-2 py-0.5 rounded-md font-black">{items.length} ITEMS</span>
          </div>
          <div className="flex gap-3">
             ...
             <button onClick={exportCSV} className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black transition-all shadow-xl active:scale-95 border-b-2 border-red-900" style={{ backgroundColor: 'var(--accent)', color: 'white' }}>
               <Download size={14} /> DOWNLOAD TAKEAWAY
             </button>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto no-scrollbar pb-32 md:pb-6 px-0 md:px-2">
          ...
          {items.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-zinc-700 p-8 text-center">
              ...
              <p className="text-sm font-medium opacity-20 italic">Start by adding items from the panel</p>
            </div>
          )}
        </div>

        <div className="md:hidden absolute bottom-0 inset-x-0 shrink-0 bg-zinc-900 border-t border-zinc-800 shadow-2xl z-40 pb-safe">
          <ItemEntry 
            onAdd={addItem} 
            types={windowTypes} 
            transoms={transomOptions} 
      ...
      <ColorPickerModal 
        ...
        secondaryColor={manualComplement}
        setSecondaryColor={setManualComplement}
        isAuto={isAutoComplement}
        setIsAuto={setIsAutoComplement}
      />
      ...
      <ConfigSettingsModal
        ...
        types={windowTypes}
        setTypes={setWindowTypes}
        transoms={transomOptions}
        setTransoms={setTransomOptions}
      />
    </div>
  );
};
